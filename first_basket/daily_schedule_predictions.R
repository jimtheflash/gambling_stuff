...
today_games_jumper <-
  ...
  mutate(home_exp_win = (home_rating*(1-away_rating)) / 
         ...
         odds = case_when(exp_win > .5 ~ (exp_win / (1 - (exp_win))) * -100,
                          TRUE ~ (100/exp_win) - 100))

ordered_df <-
  today_games_jumper %>%
  select(home_team_abbrev, home_jumper, home_wins, home_jumps, home_win_rate, 
         away_team_abbrev, away_jumper, away_wins, away_jumps, away_win_rate,
         home_rating, away_rating, winning_jumper, exp_win, odds) %>%
  filter(home_jumper %in% current_centers$player,
         away_jumper %in% current_centers$player)

team_odds <-
  ordered_df %>%
  mutate(team_win_tip = if_else(home_jumper == winning_jumper, exp_win, 1 - exp_win)) %>%
  select(team_abbrev = home_team_abbrev, team_win_tip) %>%
  bind_rows(ordered_df %>%
              mutate(team_win_tip = if_else(away_jumper == winning_jumper, exp_win, 1 - exp_win)) %>%
              select(team_abbrev = away_team_abbrev, team_win_tip))

first_shot_odds_df <-
  first_shot %>%
  filter(team_abbrev %in% team_odds$team_abbrev) %>%
  left_join(team_odds) %>%
