...
test_df_exp_win_master <- data.frame()
...
for (i in 1:length(unique_dates$game_date)) {
  print(unique_dates$game_date[i])
  ...
  player_list_df <-
    expected_win_prob %>%
    group_by(jumper = home_team_jumper, exp_win = home_win_rate) %>%
    summarise(jumps = n(),
              .groups = 'drop') %>%
    bind_rows(expected_win_prob %>%
                group_by(jumper = away_team_jumper, exp_win = away_win_rate) %>%
                summarise(jumps = n(),
                          .groups = 'drop')) %>%
    group_by(jumper, exp_win) %>%
...
calculate_jump_odds <- function(player_1, player_2, player_list_df){
  ...
  if (player_1_win >= .5) {
    print(paste0(player_1, " wins a tip vs ", player_2, " ", player_1_win * 100, " percent of the time"))
  }else{
    print(paste0(player_2, " wins a tip vs ", player_1, " ", (1 - player_1_win)*100, " percent of the time"))
  }
}

player_1 <- "Cody Zeller"
player_2 <- "Enes Kanter"
