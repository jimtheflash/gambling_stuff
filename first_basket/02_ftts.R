...
# Read in all game csvs
file_list <- list.files(path="./data/nba_lines/")
line_info_df <- tibble()

for (i in 1:length(file_list)){
  # Read in file for each player
  temp_line <- read.csv(paste0("./data/nba_lines/", file_list[i]))
  # Bind the new data to the dataset
  line_info_df <- rbind.data.frame(line_info_df, temp_line)
}


line_info_df <- fread("./data/nba_lines/all_historical_lines.csv")
...
score_first_relevant_cols <-
  ...
  select(season, game_date, game_id, matchup, home_team_abbrev,
         away_team_abbrev, home, possession, score_first, home_won_tip, home_score_first,
         final_win_prob_iterative, final_win_prob_ranger, final_win_prob_glment,
         final_win_prob_earth, final_win_prob_all)

score_first_original <-
  score_first_relevant_cols %>%
  mutate(exp_score_first_iterative = (final_win_prob_iterative*.61) + ((1 - final_win_prob_iterative)*.41),
         exp_score_first_all = (final_win_prob_all*.61) + ((1 - final_win_prob_all)*.41))

# Determines Brier Score for score first backtesting
brier_score_first_df <-
  score_first_original %>%
  mutate(brier_score_iterative = (exp_score_first_iterative - home_score_first)^2,
         brier_score_all = (exp_score_first_all - home_score_first)^2)

message("brier score of score first iterative is equal to ", round(mean(brier_score_first_df$brier_score_iterative), 5))
...
function_to_run_models <- function(possession_train_model, possession_test_model, model_desc){
  ...
  preds <-
    ...
    select(season, game_date, game_id, matchup, period, period_clock, jumper, opp_jumper, home,
           won_tip, iterative_win_tip, win_tip_prob_ranger) %>%
    cbind.data.frame(win_tip_prob_glmnet = preds_glmnet$win_tip_prob_glmnet, 
                     win_tip_prob_earth = preds_earth$win_tip_prob_earth,
                     model = model_desc)
