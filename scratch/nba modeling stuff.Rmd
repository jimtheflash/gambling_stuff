...
We're doing this in `tidyverse` to spite Shook, and also to try and grab the attention of the fine folks at RStudio in case they're looking for someone to try to liase (SP?) with sportsy people, though tbqh they already have those connections and more. AHEM.
...
## timetk to handle some rolling stuff
...
team_games1 <- player_games %>%
  ...
  dplyr::group_by(team_name, game_id, home_away, strata_var, join_var) %>%
  dplyr::summarise(
  ...
  dplyr::ungroup() %>%
  # we're going to have to manually calculate the window functions and aggregates and whatever, since we'll want to join this to itself later to get the opponent's stats and it's easier to do it now than later in some recipe or whatever
  # dplyr::arrange(team_name, game_date) %>%
  # dplyr::group_by(team_name) %>%
  # dplyr::mutate(
  #   dplyr::across(
  #     dplyr::ends_with('_for'),
  #     list(
  #       rollmean_3 = ~rollmean_p(.x, 3),
  #       rollmax_3 = ~rollmax_p(.x, 3),
  #     )
  #   )
  # ) %>%
  identity()
...
#     values_from = dplyr::ends_with('_for'),
#     names_glue = "{home_away}_{.value}"
#   )

# join it back to itself to get opponent stuff
rename_points_against <- function(x) {
  gsub('_for', '_against', x)
}
...
team_games2 <- team_games1 %>%
  dplyr::inner_join(
    team_games1 %>%
      ...
      dplyr::rename_all(rename_points_against),
...
train_test_split <- rsample::initial_split(team_games2, strata = 'strata_var')
team_games_train <- rsample::training(train_test_split)
team_games_test <- rsample::testing(train_test_split)

recipe_points_for <- recipes::recipe(pts_for ~ ., data = team_games_train %>% dplyr::select(-strata_var))
